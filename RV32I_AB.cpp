...
/***************************************** MAIN FUNCTION FOR RISCV SIMULATOR *****************************************/
		...
		int ex_lat;
		int war = 0;
int placement_logic(int a, int k[], int instr_arr[], int i, uint32_t pre_arr[][SCHEDULE_WIDTH-1]) {
		//int placement_logic(int a, int i) {
...
void print_arr(int size){
        //cout << "***********************" << endl;
		//cout << arr[size]->opcode << "\n" << endl; // Print each element followed by a space
//if((sizeof(arr)/sizeof(InstrFields)) == 10) {
	...
	for(int i=0; i<MAX_INSTRUCTIONS; i++) {
		...
		for(int j=i+1; j<(j+ex_lat); j++) {
			...
			if(rd_arr[i] == (rs1_arr[j] | rs2_arr[j])){
				printf("There is RAW data dependency between instruction %d and %d\n", i , j);
				dependency = 1;
			} else if((rs1_arr[i] | rs2_arr[i]) == rd_arr[j]){
					...
					uint32_t opcode = currentINSTR & 0x7F;
						...
						switch(opcode){
							case 0b0110111: { if(!UtypeDecode(&Fields,currentINSTR)) return 0;	
												arr[size] = &Fields;
												opcode_arr[size]=arr[size]->opcode,
												rs1_arr[size]=arr[size]->rs1,
												rs2_arr[size]=arr[size]->rs2,
												rd_arr[size]=arr[size]->rd;
												instr_arr[size]=arr[size]->instr32bit;
												print_arr(size);
												size++;
												break;	// For LUI
											}
